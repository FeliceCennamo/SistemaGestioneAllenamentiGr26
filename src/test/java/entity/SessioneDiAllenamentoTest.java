...
class SessioneDiAllenamentoTest {
    ...
    private void creaDatiDiProva() {
        allenatore = gp.salva(new Allenatore("Marco", "Rossi", TEST_MAIL_ALL, "pass", "Calcio"));
        atleta = gp.salva(new Atleta("Luca", "Bianchi", TEST_MAIL_ATL, "pass", "Corsa", 2));
        esercizioRip = gp.salva(new Esercizio(TEST_ESERCIZIO_RIP, "Descrizione rip", 10));
        esercizioTempo = gp.salva(new Esercizio(TEST_ESERCIZIO_TEMPO, "Descrizione tempo", Duration.ofMinutes(5)));
    ...
    void testRegistraRisultato_Ripetizioni() {
        ...
        eseguiInTransazione(() -> {
            SessioneDiAllenamento s = gp.trovaPerId(SessioneDiAllenamento.class, sessione.getId());
